...
function ret = d_loss_by_d_model(model, data, wd_coefficient)
  ...
  ret.input_to_hid = (1/m)*(model.hid_to_class'*(class_prob - data.targets)).* (hid_output-hid_output.^2)*data.inputs'+wd_coefficient.*model.input_to_hid;
  ret.hid_to_class = (1/m)*(class_prob - data.targets)*hid_output'+wd_coefficient.*model.hid_to_class;
end
